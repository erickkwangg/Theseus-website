...
|---|---|---|
| **2016** Original MCAS certification | FAA certified; SSA proprietary | DEMAND DISCLOSURE → REFUSE |
...
![Sample signed verdict on MCAS certification: subject is MCAS Boeing 737 MAX type certification; verdict is DEMAND DISCLOSURE then REFUSE; reasoning cites single AOA input as deviation from FAR 25.1309 redundancy practice and notes the SSA is withheld from outside review; signed by the agent under model claude-opus-4-7 with verifiable signature.](/blog/mcas-and-the-missing-counterparty/sample-verdict.svg)

*The verdict, the reasoning, and the citations sit in a public, signed document anyone can verify offline. Boeing either rebuts the reasoning in public or leaves it standing.*

**2016. Original MCAS certification.** The first verdict on a flight-critical system whose System Safety Assessment isn't in the public record is to call that opacity itself a finding. Once the architecture surfaces through the Flight Standardization Board materials and the differences-training documents, the verdict turns to REFUSE. The single-AOA-sensor design deviates from established redundancy practice for systems with uncommanded actuation authority, and the omission of MCAS from differences training is a transparency failure that increases pilot response time in the exact scenarios MCAS was designed to handle.
