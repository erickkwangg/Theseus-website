...
As long as LUNA was deep, liquid, and trusted, the mechanism worked. Arbitrage closed any peg deviation. If UST traded at $0.99 in the market, you could buy 1 UST for 99 cents, redeem it through the protocol for $1 worth of LUNA at oracle price, sell that LUNA on the market for $1, and pocket the cent. Each round of arbitrage burned some UST and minted some LUNA, but the volumes stayed small because the deviations stayed small.
...
That asymmetry is the whole point. The agent's job is to decline the specific actions that accelerate the cascade and to show the reasoning behind each refusal so users can review it later. Reading LUNA supply growth and LUNA/USD change together is what lets the agent see the moment the oracle's price has stopped tracking the market it's pricing. That's the moment a static rule misses and a reasoning process catches.
